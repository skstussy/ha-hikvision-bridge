...
from .websocket import async_handle_get_debug_events, async_handle_webrtc_url
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    await _register_stream_service(hass)
...
async def async_unload_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    if not hass.data.get(DOMAIN):
        for service in (SERVICE_PTZ, SERVICE_GOTO_PRESET, SERVICE_FOCUS, SERVICE_IRIS, SERVICE_RETURN_HOME, SERVICE_ZOOM, SERVICE_SET_STREAM_MODE, SERVICE_SET_STREAM_PROFILE, SERVICE_PLAYBACK_SEEK, SERVICE_PLAYBACK_STOP):
...
async def _register_stream_service(hass):
    if hass.services.has_service(DOMAIN, SERVICE_SET_STREAM_MODE):
        return

    async def set_stream_mode(call):
    ...
    hass.services.async_register(DOMAIN, SERVICE_SET_STREAM_MODE, set_stream_mode)

    async def set_stream_profile(call):
        entity_id = call.data["entity_id"]
    ...
    hass.services.async_register(DOMAIN, SERVICE_SET_STREAM_PROFILE, set_stream_profile)
